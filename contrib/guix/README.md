# Bootstrappable Techcoin Core Builds

This directory contains the files necessary to perform bootstrappable Techcoin
Core builds.
...
To build Techcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Techcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/techcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/techcoin-core/guix.sigs).
