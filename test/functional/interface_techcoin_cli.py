...
# The block reward of coinbaseoutput.nValue (50) TLC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 TLC/block.
...
class TestTechcoinCli(TechcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `techcoin-cli getblockchaininfo`")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and techcoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (TLC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (TLC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 TLC) to the default wallet.
            ...
            # Test techcoin-cli -generate with -rpcwallet in multiwallet mode.
