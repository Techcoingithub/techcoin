...
#include <qt/techcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Techcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TechcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new TechcoinAddressCheckValidator(parent));
}
...
bool parseTechcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no techcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("techcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TechcoinUnits::parse(TechcoinUnits::TLC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTechcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTechcoinURI(uriInstance, out);
}

QString formatTechcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("techcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TechcoinUnits::format(TechcoinUnits::TLC, info.amount, false, TechcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTechcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", TECHCOIN_CONF_FILENAME));
    ...
    /* Open techcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Techcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Techcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Techcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Techcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "techcoin.desktop";
    return GetAutostartDir() / strprintf("techcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a techcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Techcoin\n";
        else
            optionFile << strprintf("Name=Techcoin (%s)\n", chain);
