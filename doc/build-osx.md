...
This guide describes how to build techcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Techcoin Core from source.
...
### 4. Clone Techcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Techcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `techcoind` or  `techcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/techcoin/techcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Techcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/techcoin/techcoin/issues/7714).
...
You can deploy a `.dmg` containing the Techcoin Core application using `make deploy`.
...
## Building Techcoin Core

### 1. Configuration

There are many ways to configure Techcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Techcoin Core:
...
## Running Techcoin Core

Techcoin Core should now be available at `./src/techcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/techcoin-qt`.

The first time you run `techcoind` or `techcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Techcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Techcoin"

touch "/Users/${USER}/Library/Application Support/Techcoin/techcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Techcoin/techcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Techcoin/debug.log
```

## Other commands:

```shell
./src/techcoind -daemon      # Starts the techcoin daemon.
./src/techcoin-cli --help    # Outputs a list of command-line options.
./src/techcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/techcoin-qt -server # Starts the techcoin-qt server mode, allows techcoin-cli control
```
