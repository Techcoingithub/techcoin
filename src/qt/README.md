This directory contains the source code for the Techcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Techcoin UI.
...
- ClientModel is used by the main application `techcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a techcoin:... link from a browser).
...
* UI elements like TechcoinAmountField, which inherit from QWidget.
* `techcoinstrings.cpp`: automatically generated
* `techcoinunits.(h/cpp)`: TLC / mTLC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/techcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `techcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `techcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `techcoin-qt`
9. You're all set! Start developing, building, and debugging the Techcoin Core GUI
