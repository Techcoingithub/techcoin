# User-space, Statically Defined Tracing (USDT) for Techcoin Core

Techcoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Techcoin Core
...
Multiple tools can list the available tracepoints in a `techcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Techcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/techcoind
stap net        outbound_message 0x0000000000107c05 /src/techcoind
stap validation block_connected  0x00000000002fb10c /src/techcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Techcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/techcoind | grep NT_STAPSDT -A 4 -B 2
