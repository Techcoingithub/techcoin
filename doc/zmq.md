...
The Techcoin Core daemon can be configured to act as a trusted "border
router", implementing the techcoin wire protocol and relay, making
...
The ZeroMQ feature in Techcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building techcoind:
...
For instance:
    ...
    $ techcoind -zmqpubhashtx=tcp://127.0.0.1:28862 \
               -zmqpubhashtx=tcp://192.168.1.2:28862 \
               -zmqpubrawtx=ipc:///tmp/techcoind.tx.raw \
...
These options can also be provided in techcoin.conf.
...
From the perspective of techcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into techcoind directly. Furthermore, no information is
...
using. Techcoind appends an up-counting sequence number to each
